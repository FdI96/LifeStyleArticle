module ApplicationHelper
  ...
  def show_sign_out
    html = ''
    html << link_to(button_to('Sign out', destroy_user_session_path, method: :delete, class: 'btn-to-link')) if user_signed_in?
    html.html_safe
  end
  ...
  def show_lay_cat(categories)    
    if user_signed_in?
      ...
      categories.each do |category|
        html << "<span class='m-3'>"
        html << link_to(link_to("#{category.name}", category, class: 'text-white text-decoration-none'))
        html << '</span>'
      end
      html << "</div>"
      html.html_safe     
  ...
  def display_cat(categories)
    ...
    categories.each do |category|
      ...
      if user_signed_in?
        html << link_to(link_to("#{category.name}", category, class: 'text-white underline-yellow'))
      else
        html << link_to(link_to("#{category.name}", new_user_session_path, class: 'text-white underline-yellow'))
      end
      html << "</h5>"
  ...
  def display_arts(category)
    html = ''
    category.articles.with_attached_image.includes([:author, :votes]).each do | article |
      ...
      html << link_to("#{article.title}", article, class:'text-warning')
      html << "</h5>"
